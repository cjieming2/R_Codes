...
setwd('C:/Users/Jieming/Documents/thesis/lynne_work/motifVar/ank/seqID/')
domain = 'anks.30aa'
snvs = read.table('ANKs.canonical.228.SNVs.auto.txt', header=T, stringsAsFactors = F)
...
## data
filename = 'final.merged.ExAC.ens73.ANK.30aa.vepoutput.coding.canonical.noORF.sorted.auto.924.reduseq.seqIDmat'
...
setwd("C:/Users/Jieming/Documents/thesis/lynne_work/motifVar/ank/seqID/seqID_indiv")
## process a nxn symmetric matrix of sequence identities
## and returns the lower triangular as a vector n*(n+1) / 2 rows * 1
...
mtext("ANK proteins",side=1, line=2, cex=1.5)
mtext("seq id",side=2, line=2, cex=1.5)

## check correlation btw number of SNVs and seq ID
snvs.seqid = cbind(sapply(flist,mean),frows)
snvs.seqid.noNA = snvs.seqid[complete.cases(snvs.seqid),]
cor(snvs.seqid.noNA)
...
## put up empty bars
newhfcounts = c(0,hf$counts,rep(0,(length(a$mids)-1-length(hf$mids))))
newhfmids = c(0.01,hf$mids,seq(max(hf$mids)+0.02,1,0.02))
barplot(newhfcounts/sum(hf$counts),names.arg=newhfmids,main=paste('distrib mean % sequence identity within ANK proteins'),
...
legend(33,0.2,c("within ank proteins","between all ank proteins"),
