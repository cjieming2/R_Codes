...
setwd("C:/Documents and Settings/chenjm/Desktop/work_documentation/chinese population analysis/surnames/2-pca on subsets")
...
annot <- read.table("6217samples-chinese-surnames-w-genotypes.sa", header=TRUE, sep="\t")

baseFile = "db126-6217samples-373383snps-allsamples.tg-snpset"

for (i in 1:3)
{
	### pca data loading pca file
	filename= paste(baseFile, i, ".txt.pca", sep="")
	...
	evalFile = paste(baseFile, i, ".txt.eval", sep="")
	...
	pca.plot(j, m, xlab=xlabel, ylab=ylabel, main=filename, colors, data$surname)
	...
	pairs(data[,5:9], main="", pch = 20, col = colors[unclass(factor(data$surname))])
