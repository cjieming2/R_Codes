...
siftscoreCol = 75
tt.sift=table(data[,resNumCol], data[,siftscoreCol])

## convert table counts of SNVs at each position to dataframe; nonmissing
sift.nonmissData=data[complete.cases(data[,siftscoreCol]),]
sift.nonmissData[,siftscoreCol] = sift.nonmissData[,siftscoreCol]
...
do.call(vioplot.las2,c(sift.ordered,col="lightgrey",border="lightgrey",pchMed=19,cexMed=1.3,
                       colMed="black",list(names=order[,1])))
mtext("position",side=1, line=2.5, cex=1.5)
mtext("SIFT",side=2, line=2.5, cex=1.5)
...
#### UNORDERED ###
sift.unordered = array(list(NULL), c(maxResNum,1))
ctr = 1
for(i in 1:max(order[,1]))
{
  sift.unordered[[ctr]] = sift.nonmissData[,siftscoreCol][sift.nonmissData[,lastcol] == i] 
  ctr = ctr + 1
}
...
# par(mfrow=c(2,2),tcl=-0.5)
par(cex.axis=1.5,cex.lab=1.5)
do.call(vioplot.las2,c(sift.unordered,col="lightgrey",border="lightgrey",pchMed=19,cexMed=1.3,
                       colMed="black",list(names=1:max(order[,1]))))
mtext("TPR position",side=1, line=2.5, cex=1.5)
mtext("SIFT score",side=2, line=2.5, cex=1.5)



##### to plot ordered heatmap ####
## reformat siftscoreCol such that [0,0.05), [0.05-0.15), [0.15-0.25) ... [0.95,1]
## by using rounding off to nearest 0.1
newcol = ncol(data.ns.only) + 1
data.ns.only.heatmap = data.ns.only
data.ns.only.heatmap[,newcol] = round2(1-data.ns.only.heatmap[,siftscoreCol],1)
data.ns.only.heatmap = data.ns.only.heatmap[complete.cases(data.ns.only.heatmap[,newcol]),]

## create lookup
sift.ns = matrix(list(NULL), c(maxResNum,1))
seqrangesift.ns = as.data.frame(seq(0,1,0.1),row.names=as.character(seq(0,1,0.1)))
names(seqrangesift.ns) = "sift"
sift.ns.freq = matrix(0,ncol=maxResNum,nrow=nrow(seqrangesift.ns))
for(i in 1:maxResNum)
{  
  ## create freq table from each set of scores for each TPR pos i  
  ## sift-NS
  sift.ns[[i]] = data.ns.only.heatmap[,newcol][data.ns.only.heatmap[,resNumCol] == i]
  temp30.ns = as.data.frame(prop.table(table(sift.ns[[i]])))
  names(temp30.ns) = c("sift","freq") ## set the name to the same as seqrange30 for lookup
  temp30.ns = merge(seqrangesift.ns,temp30.ns,by="sift",all.x=TRUE)
  temp30.ns[is.na(temp30.ns)] = 0
  temp30.ns = as.data.frame(temp30.ns)
  sift.ns.freq[,i] = temp30.ns$freq
...
x11()
heatmap.2(sift.ns.freq[c(nrow(sift.ns.freq):1),c(1:max(order[,1]))],trace="none",key=TRUE,Rowv=NULL,Colv=NULL,dendrogram="none",
          labCol=1:max(order[,1]),labRow=rev(seqrangesift.ns$sift), col=cols, breaks=bks,
          colsep=1:34,rowsep=1:25,sepcolor="white",cexRow=1.5,cexCol=1.5,
          lhei=c(1,5),lwid=c(1,5),
          key.title=NA,key.xlab=NA,key.ylab=NA,
          xlab="position", ylab="1-SIFT NS", cex=1.5)
