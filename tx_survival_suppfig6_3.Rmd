...
data.outcome <- read.delim("/Users/chenj220/Documents/ucsf/transplantation/a_donor/immport/survival/trash/final.iTx.posttx_outcomes.v5.kidney.txt", header = T, sep = "\t", stringsAsFactors = F)
...
death_kidney <- read.delim("/Users/chenj220/Documents/ucsf/transplantation/a_donor/immport/survival/final.iTx.posttx_outcomes.v6.txt", stringsAsFactors = F, header = T, sep = "\t") %>% 
  filter(Organ == "kidney") %>%
...
## (1) all
data.outcome.nw.surv <- data.outcome.nw.surv %>%
  mutate(time_final = ifelse(death == 1 & !is.na(NDIDEATH_MAX), NDIDEATH_MAX, time))
data.outcome.surv.table <- Surv(data.outcome.nw.surv$time_final/365, data.outcome.nw.surv$event) ## survival table
fit <- survfit(data.outcome.surv.table~1)
summary(fit)
# pdf("/Users/chenj220/Documents/ucsf/transplantation/manuscript/figures/suppfig7-survival/survival2/surv_withAllLKDs_plain.pdf")
plot(fit, xlab="t", ylab=expression(hat(S)*"(t)"), ylim = c(0.96,1)) ## base plot
# dev.off()
# pdf("/Users/chenj220/Documents/ucsf/transplantation/manuscript/figures/suppfig7-survival/survival2/surv_withAllLKDs_ggsurv_zoomedin.pdf")
ggsurvplot(fit, data = data.outcome.nw.surv, risk.table = T, ylim = c(0.96,1), pval = T,
           censor = T) ## survminer
# dev.off()


# pdf("/Users/chenj220/Documents/ucsf/transplantation/manuscript/figures/suppfig7-survival/survival2/surv_withAllLKDs_ggsurv.pdf")
ggsurvplot(fit, data = data.outcome.nw.surv, risk.table = T, pval = T,
           censor = T) ## survminer
# dev.off()
```
...
summary(fit)
# pdf("/Users/chenj220/Documents/ucsf/transplantation/manuscript/figures/suppfig7-survival/survival2/surv_subsets.pdf")
ggsurvplot(
  fit, 
  xlab = 'Time (years)',
  data = do.z.1dz.surv, 
...
do.multiple.surv <- do.multiple.surv %>%
  mutate(time_final = ifelse(death == 1 & !is.na(NDIDEATH_MAX), NDIDEATH_MAX, time)) %>%
  mutate(multipleDiseases_final = ifelse(multipleDiseases == 3 | multipleDiseases == 4 | multipleDiseases == 5, 345, multipleDiseases))
...
summary(fit)
# pdf("/Users/chenj220/Documents/ucsf/transplantation/manuscript/figures/suppfig7-survival/survival2/surv_multipledzs.pdf")
ggsurvplot(
  fit, 
  xlab = 'Time (years)',
...
do.multiple.surv.table <- Surv(do.multiple.surv$time_final/365, do.multiple.surv$event) ## survival table
fit <- survfit(do.multiple.surv.table ~ multipleDiseases_final, data = do.multiple.surv)
summary(fit)
# pdf("/Users/chenj220/Documents/ucsf/transplantation/manuscript/figures/suppfig7-survival/survival2/surv_multipledzs_combined345.pdf")
ggsurvplot(
  fit, 
  xlab = 'Time (years)',
  data = do.multiple.surv, 
