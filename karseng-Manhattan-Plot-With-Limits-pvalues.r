#-------------------------
# Script to generate manhattan plot of p values
#-------------------------
...
corr=corr_temp[,c(corr_col_snpid,corr_col_pval)]
...
data_temp = merge(marker, result, by.x = 1, by.y = 1, sort=F)
#data_temp$logP = -1 * log10(data_temp$P)
...
#-------------------------
# Calculate relative chromosome position and offser
#-------------------------
...
for (j in 4:7)
{
	...
	points(data[overLimit,"plotX"], rep(minuslogPlimit, time=numOverLimit), pch=24, col=color[data[overLimit,"chromosome"]], bg=color[data[overLimit,"chromosome"]])
