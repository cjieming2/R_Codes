...
## donors ####
data.donor = read.table("emr_ucsf_2016jun_kidney_donors_with_inpatient_bill_526subj.txt", header = T, sep = "\t")
a.d = data.donor
b1.d=ddply(.data = a.d, .variables = c("Billing_Age","Patient_Sex"), .fun=summarise, count = length(Billing_Age))
b3.d=ddply(.data = a.d, .variables = c("Billing_Age","Patient_Sex","Patient_Race2"), .fun=summarise, count = length(Billing_Age))

## plot and save
plot_age_sex = function(data,flag,range) {
  names(data)[names(data)=="Patient_Sex"]  <- "Sex"
  names(data)[names(data)=="Billing_Age"]  <- "AGE"
...
# savePlot("emrjun2016_ucsf_age_sex_histogram_kidney_donors.png", type = "png")
...
b.d.age = ddply(.data = a.d, .variables = c("Billing_Age","Patient_Race2"), .fun=summarise,
                  count = length(Billing_Age))
pmain.age = ggplot(data=b.d.age, aes(x=Billing_Age, y=count, fill=Patient_Race2))
...
# savePlot("emrjun2016_ucsf_age_race_sex_ethnicity_histogram_kidney_donors.png", type = "png")
