setwd("/Users/jiemingchen/Documents/scripts-R_perl_shell_macros/toolboxes/R_codes/PubMed/PubMedTrend/")
library(XML)
library(httr)
library(ggplot2)

url = "http://www.nlm.nih.gov/bsd/medline_cit_counts_yr_pub.html"
tabs <- GET(url)

# create total_table --------------------------------------------------
total_table <- readHTMLTable(rawToChar(tabs$content), stringsAsFactors = F)
total_table <- total_table[1]
total_table <- as.data.frame(total_table)
total_table <- total_table[c(1,2)]
total_table <- total_table[-(70:72),] ## drop last few rows with unclear years
...
getTotalUpdated <- function(yrStart=1950, yrMax=2016) {
  # add progressbar
...
total_table_updated <- getTotalUpdated(1947,2015) ## yrMax has to match the old one
...
#load(file="total_table.csv")
#load(file="total_table_updated.csv")
