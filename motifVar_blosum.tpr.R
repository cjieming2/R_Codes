...
## create lookup
seqrange30 = as.data.frame(seq(min(data$V77),max(data$V77)),row.names=seq(min(data$V77),max(data$V77))); names(seqrange30) = "blosum30"
seqrange30.ns = as.data.frame(seq(min(data.ns.only$V77),max(data.ns.only$V77)),row.names=seq(min(data.ns.only$V77),max(data.ns.only$V77))); names(seqrange30.ns) = "blosum30"
seqrange62.ns = as.data.frame(seq(min(data.ns.only$V77),max(data.ns.only$V77)),row.names=seq(min(data.ns.only$V77),max(data.ns.only$V77))); names(seqrange62.ns) = "blosum62"
...
for(i in order[,1])
{
  blos30.ordered[[ctr]] = data$V77[data[,lastcol] == i] 
  blos62.ordered[[ctr]] = data$V66[data[,lastcol] == i]
  blos30.ns.ordered[[ctr]] = data.ns.only$V77[data.ns.only[,lastcol] == i] 
  blos62.ns.ordered[[ctr]] = data.ns.only$V66[data.ns.only[,lastcol] == i] 
  ctr = ctr + 1
}
