setwd("/Users/jiemingchen/Documents/transplantation/a_recipient/emr")
...
## donors ####
data.recipient = read.table("emr_ucsf_2016jun_kidney_recipients_no_billing_2763subj.txt", header=T, sep="\t", stringsAsFactors = F)
a.d = data.recipient

b1.d=ddply(.data = a.d, .variables = c("Patient_Age","Patient_Sex"), .fun=summarise, count = length(Patient_Age))
b3.d=ddply(.data = a.d, .variables = c("Patient_Age","Patient_Sex","Patient_Race2"), .fun=summarise, count = length(Patient_Age))

## plot and save
plot_age_sex = function(data,flag,range) {
  names(data)[names(data)=="Patient_Sex"]  <- "Sex"
  names(data)[names(data)=="Patient_Age"]  <- "AGE"
...
# savePlot("emrjun2016_ucsf_age_sex_histogram_kidney_recipients_withNoBilling_.png", type = "png")
...
b.d.age = ddply(.data = a.d, .variables = c("Patient_Age","Patient_Race2"), .fun=summarise,
                  count = length(Patient_Age))
pmain.age = ggplot(data=b.d.age, aes(x=Patient_Age, y=count, fill=Patient_Race2))
...
# savePlot("emrjun2016_ucsf_age_race_sex_ethnicity_histogram_kidney_recipients_withNoBilling.png", type = "png")
