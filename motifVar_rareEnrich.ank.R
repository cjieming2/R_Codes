setwd('C:/Users/Jieming/Documents/thesis/lynne_work/motifVar/ank')
source("C:/Users/Jieming/Documents/Shared/scripts-R_perl_shell_macros/R codes/jmRlib.R")

domain = 'ExAC ANKs'

## data
data = read.table('final.merged.ExAC.ens73.ANK.30aa.vepoutput.coding.canonical.noORF.sorted.auto.enrich', sep="\t", header=T, stringsAsFactors = F)
data[data==0.001] <- 0
...
lastCol = ncol(data) + 1
data[,lastCol] = factor(data$resNum, levels=levels(data$resNum)<-order$pos)
...
############################################ plot bar plots
##### plot 4 bar plots rare ns comm ns rare s comm s
rare.comm.ns.noS = data$numRareNS.noS + data$numCommNS.noS
yuplimit = max(data$totVar.noS)+100
x11()
par(cex.axis=1, cex.lab=2, cex.main=2)

##plot total bar noS (rep comm s)
barplot(data$totVar.noS,names.arg=data$resNum,main=paste('variants by ', domain, ' motif pos'),
        col=add.alpha('cyan',1), ylim=c(0,yuplimit),las=2)
## plot comm ns+ rare s + rare ns
rare.comm.ns.noS.rareS = data$numRareNS.noS + data$numCommNS.noS + data$numRareS.noS
barplot(rare.comm.ns.noS.rareS,add=T,xaxt="n",col=add.alpha('blue',1),ylim=c(0,yuplimit), las=2)

## plot rare s + rare ns
rare.s.ns.noS = data$numRareS.noS + data$numRareNS.noS
barplot(rare.s.ns.noS,add=T,xaxt="n",col=add.alpha('pink',1),ylim=c(0,yuplimit), las=2)

## plot rare ns
barplot(data$numRareNS.noS,add=T,xaxt="n",col='red',ylim=c(0,yuplimit), las=2) # tab.


legend(6,yuplimit,c("comm.s.noS","comm.ns.noS","rare.s.noS","rare.ns.noS"),
       col=c("cyan","blue","pink","red"), cex=2, pt.cex=2,
...
barplot(rare.comm.ns.noS,names.arg=data$resNum,main=paste('variants by ', domain, ' motif pos'),
        col='blue', ylim=c(0,yuplimit),las=2)
barplot(data$numRareNS.noS,add=T,xaxt="n",col='red',ylim=c(0,yuplimit), las=2) # tab.var.noSingle
...
barplot(rare.comm.s.noS,names.arg=data$resNum,main=paste('variants by ',domain,' motif pos'),
        col='cyan',ylim=c(0,yuplimit), las=2)
barplot(data$numRareS.noS,add=T,xaxt="n",col='magenta',ylim=c(0,yuplimit), las=2) # tab.var.noSingle


legend(12,yuplimit,c("rare.s.noS","comm.s.noS"),
...
legend(25,2,c("nsrare:srare","nscomm:scomm"),c('red','blue'))
